...
public partial class VehicleTypePage
{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			var vehicleType = await CommonData.LoadTableDataById<VehicleTypeModel>(FleetNames.VehicleType, id)
				?? throw new Exception("Transaction not found.");

			if (isRecover) await VehicleTypeData.RecoverTransaction(vehicleType, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await VehicleTypeData.DeleteTransaction(vehicleType, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());

			await _toastNotification.ShowAsync("Success", $"Transaction {vehicleType.Name} has been {(isRecover ? "recovered" : "deleted")} successfully.", ToastType.Success);
			ResetPage();
		}
		catch (Exception ex)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await VehicleTypeExport.ExportMaster(_vehicleTypes, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
