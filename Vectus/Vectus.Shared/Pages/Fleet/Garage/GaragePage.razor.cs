...
public partial class GaragePage
{
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			if (isRecover) await GarageData.RecoverTransaction(garage, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await GarageData.DeleteTransaction(garage, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());

			await _toastNotification.ShowAsync("Success", $"Transaction {garage.Name} has been {(isRecover ? "recovered" : "deleted")} successfully.", ToastType.Success);
			ResetPage();
		}
		catch (Exception ex)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await GarageExport.ExportMaster(_garages, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
