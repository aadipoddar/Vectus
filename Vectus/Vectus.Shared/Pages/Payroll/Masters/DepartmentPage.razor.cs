...
public partial class DepartmentPage
{
	...
	private SfGrid<DepartmentModel> _sfGrid;
	...
	{
		_departments = await CommonData.LoadTableData<DepartmentModel>(PayrollNames.Department);

		if (!_showDeleted)
			_departments = [.. _departments.Where(d => d.Status)];
	...
	private async Task SaveTransaction()
	{
		...
		try
		{
			...
			await DepartmentData.SaveTransaction(_department, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
	...
	private async Task EditSelectedItem()
	{
		...
		_department = await CommonData.LoadTableDataById<DepartmentModel>(PayrollNames.Department, selectedRecords[0].Id);
	...
	private async Task DeleteRecoverTransaction(int id, bool isRecover)
	{
		try
		{
			...
			if (isRecover) await DepartmentData.RecoverTransaction(department, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());
			else await DepartmentData.DeleteTransaction(department, _user.Id, FormFactor.GetFormFactor() + FormFactor.GetPlatform());

			await _toastNotification.ShowAsync("Success", $"Transaction {department.Name} has been {(isRecover ? "recovered" : "deleted")} successfully.", ToastType.Success);
			ResetPage();
		}
		catch (Exception ex)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await DepartmentExport.ExportMaster(_departments, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
	...
	private async Task OnGridContextMenuItemClicked(ContextMenuClickEventArgs<DepartmentModel> args)
	{
		switch (args.Item.Id)
		{
			case "EditSelectedItem": await EditSelectedItem(); break;
