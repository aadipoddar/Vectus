...
public partial class DesignationPage
{
	...
	private DesignationModel _designation = new();
	...
	private SfGrid<DesignationModel> _sfGrid;
	...
	{
		_designations = await CommonData.LoadTableData<DesignationModel>(PayrollNames.Designation);

		if (!_showDeleted)
			_designations = [.. _designations.Where(d => d.Status)];
	...
	private async Task EditSelectedItem()
	{
		...
		_designation = await CommonData.LoadTableDataById<DesignationModel>(PayrollNames.Designation, selectedRecords[0].Id);
		if (_designation is null)
		{
	...
	private async Task ExportMaster(bool isExcel = false)
	{
		...
		try
		{
			...
			var (stream, fileName) = await DesignationExport.ExportMaster(_designations, isExcel ? ReportExportType.Excel : ReportExportType.PDF);
	...
	#region Utilities
	private async Task OnGridContextMenuItemClicked(ContextMenuClickEventArgs<DesignationModel> args)
	{
		switch (args.Item.Id)
		{
